...
@foreach ($epargnes2 as $row)
    @php
        $totalEpargne = $totalEpargne + $row['montant'];
    @endphp
@endforeach
...
@section('contentPage')
    <div class="row">
        ...
        <div class="col-md-3">
            <h3 class="ml-4"><i class="fas fa-piggy-bank nav-icon"></i> : {{ $totalEpargne }} FCFA</h2>
        </div>
    </div>
    <div class="row">
        <div class="col-md-6">
            <div class="">
                <div class="ml-4 mr-4">
                    <h3 class="mt-3 mb-3"><u>Ajouter une épargne</u></h2>
                    @if(session('status'))
                    ...
                    @endif
                    <form action="{{ route('auth.epargne.post') }}" method="post">
                        @csrf
                        ...
                        <input type="submit" class="mb-3  btn btn-primary" value="Ajouter l'épargne">
