...
@foreach ($depenses2 as $row)
    @php
        $totalDepense = $totalDepense + $row['montant'];
    @endphp
@endforeach
...
@section('contentPage')
    <div class="row">
        ...
        <div class="col-md-3">
            <h3 class="ml-4"><i class="fas fa-comments-dollar nav-icon"></i> : {{ $totalDepense }} FCFA</h2>
        </div>
    </div>
    <div class="row">
        <div class="col-md-6">
            <div class="">
                <div class="ml-4 mr-4">
                    <h3 class="mt-3 mb-3"><u>Ajouter une dépense</u></h2>
                    @if(session('status'))
                    ...
                    @endif
                    <form action="{{ route('auth.depense.post') }}" method="post">
                        @csrf
                        ...
                        <input type="submit" class="mb-3  btn btn-primary" value="Ajouter la dépense">
